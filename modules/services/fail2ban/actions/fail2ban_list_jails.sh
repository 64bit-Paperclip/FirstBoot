...
action_fail2ban_list_jails() {
    ...
    if ! is_fail2ban_running; then
        ...
        fail2ban_get_active_jails _F2B_LJ_ACTIVE
        ...
        if [ ${#_F2B_LJ_ACTIVE[@]} -eq 0 ]; then
            ...
            for _F2B_LJ_JAIL in "${_F2B_LJ_ACTIVE[@]}"; do
                ...
                _F2B_LJ_BANNED=$(fail2ban_get_jail_banned "$_F2B_LJ_JAIL")
                _F2B_LJ_FAILED=$(fail2ban_get_jail_failed "$_F2B_LJ_JAIL")
    ...
    local -a _F2B_LJ_ALL=()
    fail2ban_get_jail_names _F2B_LJ_ALL
    ...
    if [ ${#_F2B_LJ_ALL[@]} -eq 0 ]; then
        ...
        for _F2B_LJ_JAIL in "${_F2B_LJ_ALL[@]}"; do
            ...
            _F2B_LJ_FILE=$(fail2ban_get_jail_file "$_F2B_LJ_JAIL")
